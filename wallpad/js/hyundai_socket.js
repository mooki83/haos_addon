...
parser.on('data', buffer => {
  ...
  if (buffer[0] != 0xf7) {
    log('[Socket] Packet Error(Header):', buffer.toString('hex', 0, 1), buffer.toString('hex', 1));
    return;
  } else if (buffer.length+1 != buffer[1]) {
    log('[Socket] Packet Error(Length):', buffer.toString('hex', 0, 2), buffer.toString('hex', 2));
    return;
...
const commandProc = () => {
  ...
  if (obj.commandHex) {
    ...
    obj.sentCount++;
    log('[Socket] Send to Device:', CONST.DEVICE_CONFIG[obj.base_topic].name, '->', obj.state, '('+delay+'ms) ', obj.commandHex.toString('hex', 0, 7), obj.commandHex.toString('hex', 7));

    // 다시 큐에 저장하여 Ack 메시지 받을때까지 반복 실행
    if(obj.sentCount <= CONST.COMMAND_MAX_RETRY_COUNT) {
      queue.push(obj);
    } else {
      log('[Socket] Fail command:', obj.commandHex.toString('hex', 0, 7), obj.commandHex.toString('hex', 7));
    }
  } else if (obj.requestHex) {
    sock.write(obj.requestHex, (err) => { if (err) return log('[Socket] Send Error: ', err.message); });
    lastActivity = new Date().getTime();
    obj.lastDelegateActivity = lastActivity;
    log('[Socket] Delegate Scheduled Request:', obj.category, '('+delay+'ms)', obj.requestHex.toString('hex', 0, 7), obj.requestHex.toString('hex', 7) );
  } else {
